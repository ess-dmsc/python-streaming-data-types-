...
def serialise_answ(service_id: str, job_id: str, action: ActionType, outcome: ActionOutcome, message: str) -> bytes:
...
Response = namedtuple("Response", ("service_id", "job_id", "action", "outcome", "message",),)
...
def deserialise_answ(buffer: Union[bytearray, bytes]):
    ...
    return Response(answ_message.ServiceId().decode("utf-8"), answ_message.JobId().decode("utf-8"), answ_message.Action(), answ_message.Outcome(), answ_message.Message().decode("utf-8"))
